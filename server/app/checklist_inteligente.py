...
from .ai_providers import (
    ...
    get_checklist_enrichment_chain,
    get_checklist_generation_chain,
    get_checklist_page_chain,
)
from .enums import ChecklistGeracaoStatus
...
def analisar_pagina(img_b64: str, page_label: str) -> dict:
    ...
    result = call_vision_with_fallback(
        providers=get_checklist_page_chain(),
        content_parts=content_parts,
        max_tokens=2048,
        task_label=f"Analise pagina '{page_label}'",
    )
...
def gerar_itens_para_caracteristica(
    ...
    prompt = f"{PHASE2_SYSTEM_PROMPT}\n\nConsulta: {query}"

    result = call_text_with_fallback(
        providers=get_checklist_generation_chain(),
        prompt=prompt,
        task_label=f"Fase 2 ({caracteristica_id})",
    )
...
def enriquecer_item_unico(
    ...
    return call_text_with_fallback(
        providers=get_checklist_enrichment_chain(),
        prompt=prompt,
        max_tokens=2048,
        task_label=f"Enriquecimento '{titulo[:40]}'",
    )
...
def processar_checklist_background(
    ...
    try:
        ...
        with Session(engine) as session:
            log = session.get(ChecklistGeracaoLog, log_id)
            if log:
                log.status = ChecklistGeracaoStatus.CONCLUIDO
        ...
        try:
            with Session(engine) as session:
                log = session.get(ChecklistGeracaoLog, log_id)
                if log:
                    log.status = ChecklistGeracaoStatus.ERRO
