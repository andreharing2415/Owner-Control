...
def registrar_usuario(request: Request, payload: UserRegister, session: Session = Depends(get_session)):
...
def login(request: Request, payload: UserLogin, session: Session = Depends(get_session)):
    ...
    if not user or not user.password_hash:
        logger.warning("Failed login attempt for %s from %s", email, request.client.host if request.client else "unknown")
...
def refresh_token(request: Request, payload: TokenRefreshRequest, session: Session = Depends(get_session)):
...
@limiter.limit("10/minute")
def login_google(request: Request, payload: GoogleLoginRequest, session: Session = Depends(get_session)):
