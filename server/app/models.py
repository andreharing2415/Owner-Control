...
class Obra(SQLModel, table=True):
    ...
    tipo: str = Field(default="construcao")  # "construcao" | "reforma"
    created_at: datetime = Field(default_factory=_utcnow)
    updated_at: datetime = Field(default_factory=_utcnow)
...
class ChecklistItem(SQLModel, table=True):
    ...
    projeto_doc_id: Optional[UUID] = Field(default=None, index=True, foreign_key="projetodoc.id")
...
class AtividadeCronograma(SQLModel, table=True):
    """Atividade do cronograma hierárquico (nível 1 ou 2)."""
    id: UUID = Field(default_factory=uuid4, primary_key=True, index=True)
    obra_id: UUID = Field(index=True, foreign_key="obra.id")
    parent_id: Optional[UUID] = Field(default=None, index=True, foreign_key="atividadecronograma.id")
    nome: str
    descricao: Optional[str] = None
    ordem: int = Field(default=0)
    nivel: int = Field(default=1)  # 1 = macro, 2 = detalhe
    status: str = Field(default="pendente")  # pendente | em_andamento | concluida
...
class ServicoNecessario(SQLModel, table=True):
    """Serviço/profissional necessário para uma atividade do cronograma."""
    id: UUID = Field(default_factory=uuid4, primary_key=True, index=True)
    atividade_id: UUID = Field(index=True, foreign_key="atividadecronograma.id")
    descricao: str  # ex: "pedreiro", "concreto usinado"
    categoria: str  # mapeia para Prestador.subcategoria
