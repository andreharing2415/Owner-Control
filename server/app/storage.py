...
def _storage_backend() -> str:
    ...
    Priority: Supabase > S3/MinIO > GCS
    """
...
def _upload_supabase(bucket_name: str, object_key: str, file_obj: BinaryIO, content_type: str | None) -> str:
    import httpx
    url = f"{_supabase_storage_url()}/object/{bucket_name}/{object_key}"
    headers = _supabase_headers()
    if content_type:
        headers["Content-Type"] = content_type
    data = file_obj.read()
    # upsert to allow overwriting existing files
    headers["x-upsert"] = "true"
    resp = httpx.post(url, headers=headers, content=data, timeout=120)
    resp.raise_for_status()
    # Return authenticated URL (bucket is private — SEC-11)
    base = os.getenv("SUPABASE_URL", "").rstrip("/")
    return f"{base}/storage/v1/object/{bucket_name}/{object_key}"


def _download_supabase(bucket_name: str, object_key: str) -> bytes:
    import httpx
    url = f"{_supabase_storage_url()}/object/{bucket_name}/{object_key}"
    resp = httpx.get(url, headers=_supabase_headers(), timeout=60)
    resp.raise_for_status()
    return resp.content


def _ensure_bucket_supabase(bucket_name: str) -> None:
    import httpx
    url = f"{_supabase_storage_url()}/bucket/{bucket_name}"
    resp = httpx.get(url, headers=_supabase_headers(), timeout=10)
    if resp.status_code == 404:
        httpx.post(
            f"{_supabase_storage_url()}/bucket",
            headers=_supabase_headers(),
...
def ensure_bucket(bucket_name: str) -> None:
    ...
    if backend == "supabase":
        _ensure_bucket_supabase(bucket_name)
    elif backend == "s3":
...
def upload_file(bucket_name: str, object_key: str, file_obj: BinaryIO, content_type: str | None) -> str:
    backend = _storage_backend()
    if backend == "supabase":
        return _upload_supabase(bucket_name, object_key, file_obj, content_type)
    if backend == "s3":
...
def download_file(bucket_name: str, object_key: str) -> bytes:
    backend = _storage_backend()
    if backend == "supabase":
        return _download_supabase(bucket_name, object_key)
    if backend == "s3":
        return _download_s3(bucket_name, object_key)
    return _download_gcs(bucket_name, object_key)


def delete_file(bucket_name: str, object_key: str) -> None:
    """Delete a file from the active storage backend."""
    backend = _storage_backend()
    if backend == "supabase":
        import httpx
        url = f"{_supabase_storage_url()}/object/{bucket_name}/{object_key}"
        resp = httpx.delete(url, headers=_supabase_headers(), timeout=30)
        resp.raise_for_status()
    elif backend == "s3":
        _get_s3_client().delete_object(Bucket=bucket_name, Key=object_key)
    else:
        client = _get_gcs_client()
...
def extract_object_key(arquivo_url: str, bucket: str) -> str:
    """Extract the GCS/S3/Supabase object key from a stored file URL.

    Handles multiple URL formats:
    - GCS: https://storage.googleapis.com/{bucket}/{key}
    - Supabase: https://xxx.supabase.co/storage/v1/object/public/{bucket}/{key}
