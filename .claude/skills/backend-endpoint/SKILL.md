...
description: Adiciona um novo endpoint FastAPI ao backend do ObraMaster. Use quando precisar criar um novo endpoint na API Python — qualquer menção a "endpoint", "rota", "API", "backend", "servidor" ou "FastAPI" deve ativar esta skill.
disable-model-invocation: false
---

# Adicionar endpoint FastAPI — ObraMaster
...
- Repositório do backend: `C:\Project\ObraMaster\Owner-Control\server\` (no repo principal, não nesta worktree)
...
- Storage S3/MinIO: `server/app/storage.py`
- Migrações Alembic: `server/alembic/versions/`
- Flutter client (esta worktree): adicionar método em `lib/api/api.dart`
...
6. Adicionar método correspondente em `lib/api/api.dart` (Flutter client)
...
5. Adicione o método cliente em `lib/api/api.dart` para o Flutter consumir
